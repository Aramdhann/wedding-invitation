<template>
  <div v-if="isAuthenticated" class="flex flex-col items-center justify-center min-h-screen">
    ...
    <input type="file" @change="handleFileUpload" class="mt-[50px]" />
    <button @click="downloadUpdatedFile" class="mt-[50px] px-10 py-2 bg-[#B2CAB5] rounded-full duration-500 hover:bg-[#A0B8A4] hover:scale-105">
      Download Updated Excel File
    </button>
    <button @click="downloadJsonFile" class="mt-4 px-10 py-2 bg-[#B2CAB5] rounded-full duration-500 hover:bg-[#A0B8A4] hover:scale-105">Download JSON File</button>
  </div>
  <div v-else>
    <form
      @submit.prevent="login"
      class="flex flex-col items-center justify-center min-h-screen gap-5"
    >
      <input
        v-model="username"
        placeholder="Username"
        class="px-5 py-3 rounded-full focus:outline-none focus:ring-4 focus:ring-yellow-500 duration-300"
      />
      <input
        v-model="password"
        type="password"
      ...
      <button
        type="submit"
        class="px-10 py-2 bg-[#B2CAB5] rounded-full duration-500 hover:bg-[#A0B8A4] hover:scale-105 transform transition-opacitytransition-all"
      >
        Login
      </button>
    </form>
...
const processFile = (file: File) => {
  ...
  reader.onload = (e) => {
    ...
    const updatedFile = XLSX.write(newWorkbook, {
      bookType: "xlsx",
      type: "array",
    });
...
const downloadJsonFile = () => {
  if (invitedGuests.value.length > 0) {
    ...
    const jsonBlob = new Blob([JSON.stringify(jsonNames, null, 2)], {
